...
class  SVD:
    ...
    def train(self,wu,wv,data_test,steps=100,gamma=0.002,Lambda=0.01,changerate=0.975,epsilon=0.0001):
        rmse_old = 0 
        for step in range(steps):
            ...
            for j in range(self.X.shape[0]):
                ...
                mid=self.X[i][1]
                rat=int(self.X[i][2])
                int_rat = int(round(rat))
                w = wu[uid][int_rat-1]*wv[mid][int_rat-1]
                #参数更新
                eui=rat-self.pred(uid,mid)
                rmse_sum+=eui**2
                self.bu[uid]+=gamma*(eui*w-Lambda*self.bu[uid])
                self.bi[mid]+=gamma*(eui*w-Lambda*self.bi[mid])
                temp=self.qi[mid]
                self.qi[mid]+=gamma*(eui*self.pu[uid]*w-Lambda*self.qi[mid])
                self.pu[uid]+=gamma*(eui*temp*w-Lambda*self.pu[uid])
